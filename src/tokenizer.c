        ...
        if (input_string[tc.current_index] == '<') {
            ...
            while (tc.current_index < strlen(input_string))
            {
                ...
                if (tc.inside_string) {
                    ...
                    if (input_string[tc.current_index] == '\\') {
                        if ((tc.current_index + 2) < strlen(input_string)) {
                            tc.current_index = tc.current_index + 2;
                            continue;
                        }
                    }
...
/* Tokenize the first HTML element, which is usually "<!DOCTYPE html>" */
...
/* Advance the current position to the next occurrence of a specific char */
...
/* Initiate the array responsible for storing the tokens generated by the tokenizer */
...
/* Record/Insert a token into the array */
